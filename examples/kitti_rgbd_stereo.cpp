...
void saveTotalTime(const double &time, const int nImages,
                   const std::string &strSavePath);
...
void LoadImages(const std::filesystem::path &baseDir,
                std::vector<std::string> &vstrImageFilenamesRGB,
                std::vector<std::string> &vstrImageFilenamesD) {
  // Locate the `rgb` and `depth` directories
  std::filesystem::path rgbDir, depthDir;
  for (const auto &entry : std::filesystem::directory_iterator(baseDir)) {
    if (entry.is_directory() && entry.path().filename() == "rgb") {
      rgbDir = entry.path();
    } else if (entry.is_directory() && entry.path().filename() == "depth_stereo") {
      depthDir = entry.path();
    }
  }
  ...
  if (rgbDir.empty()) {
    std::cerr << "Error: 'rgb' directory not found in " << baseDir << std::endl;
    return;
  }
  if (depthDir.empty()) {
    std::cerr << "Error: 'depth' directory not found in " << baseDir
              << std::endl;
    return;
  }

  // Load RGB images
  for (const auto &imagePath : std::filesystem::directory_iterator(rgbDir)) {
    if (imagePath.is_regular_file()) {
...
void saveTotalTime(const double &time, const int nImages,
  ...
  if (!std::filesystem::exists(dirPath)) {
    std::filesystem::create_directories(
        dirPath); // Creates directory if it doesn't exist
  }

  std::ofstream out;
  out.open(strSavePath.c_str());
  out << "Total time: " << std::fixed << std::setprecision(4) << time
      << std::endl;
  out << "Number of images: " << nImages << std::endl;
  out << "FPS: " << std::fixed << std::setprecision(4) << nImages / time
      << std::endl;
  out.close();
}