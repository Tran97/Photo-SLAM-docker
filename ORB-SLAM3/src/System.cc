...
Atlas *System::getAtlas() { return this->mpAtlas; }

Tracking *System::getTracker() { return this->mpTracker; }

LocalMapping *System::getLocalMapper() { return this->mpLocalMapper; }

LoopClosing *System::getLoopCloser() { return this->mpLoopCloser; }

Settings *System::getSettings() { return this->settings_; }

FrameDrawer *System::getFrameDrawer() { return this->mpFrameDrawer; }

MapDrawer *System::getMapDrawer() { return this->mpMapDrawer; }
...
cv::Mat System::preprocessImage(const cv::Mat &src) { return src.clone(); }
...
std::vector<Sophus::SE3f> System::getPoses() {
  ...
  for (list<Sophus::SE3f>::iterator
    ...
    //*lit is the relative transformation from the current frame to its
    //reference keyframe. By multiplying it with Trw, which transforms the
    // reference keyframe to the world frame, we obtain Tcw, the pose of the
    // current frame relative to the world.
    ...
    // Eigen::Vector3f euler_angles = q.toRotationMatrix().eulerAngles(2, 1, 0);
    ...
    //             * Eigen::AngleAxisf(euler_angles[2],
