...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft,
  ...
  if (mSensor == System::STEREO && !mpCamera2)
    mCurrentFrame =
        Frame(mImGray, imGrayRight, mImRGB, imRightRGB, timestamp,
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im,
  ...
  if (mSensor == System::MONOCULAR) {
    if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET ||
      ...
      mCurrentFrame =
          Frame(mImGray, mImRGB, timestamp, mpIniORBextractor, mpORBVocabulary,
                mpCamera, mDistCoef, mbf, mThDepth);
    else
      mCurrentFrame =
          Frame(mImGray, mImRGB, timestamp, mpORBextractorLeft, mpORBVocabulary,
                mpCamera, mDistCoef, mbf, mThDepth);
...
void Tracking::UpdateLastFrame() {
  ...
  for (size_t j = 0; j < vDepthIdx.size(); j++) {
    ...
    if (bCreateNew) {
      Eigen::Vector3f x3D,
          colorRGB; // colorRGB: actually not used because pNewMP is a temporary
                    // MapPoint, which will not be added to the Map
