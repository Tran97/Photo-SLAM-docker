...
void GaussianMapper::readConfigFromFile(std::filesystem::path cfg_path) {
  ...
  isReplica_ = (settings_file["IsReplica"].operator int()) != 0;

  isTUM_ = (settings_file["IsTUM"].operator int()) != 0;
...
void GaussianMapper::run() {
  ...
  while (!isStopped()) {
    std::chrono::steady_clock::time_point initialMapping1 =
    ...
    std::chrono::steady_clock::time_point initialMapping2 =
        std::chrono::steady_clock::now();
    double timeInitialMapping =
        std::chrono::duration_cast<std::chrono::duration<double>>(
            initialMapping2 - initialMapping1)
            .count();
  ...
  while (!isStopped()) {
    std::chrono::steady_clock::time_point incrementalMapping1 =
        std::chrono::steady_clock::now();
    ...
    if (SLAM_ended_ || getIteration() >= opt_params_.iterations_)
      break;
    std::chrono::steady_clock::time_point incrementalMapping2 =
        std::chrono::steady_clock::now();
    double timeInitialMapping =
        std::chrono::duration_cast<std::chrono::duration<double>>(
            incrementalMapping2 - incrementalMapping1)
            .count();
  ...
  while (getIteration() - SLAM_stop_iter <= n_delay_iters ||
         ...
         isKeepingTraining()) {
    std::chrono::steady_clock::time_point tailOptimizationTime1 =
        std::chrono::steady_clock::now();
    ...
    std::chrono::steady_clock::time_point tailOptimizationTime2 =
        std::chrono::steady_clock::now();
    double timeTailOptimization =
        std::chrono::duration_cast<std::chrono::duration<double>>(
            tailOptimizationTime2 - tailOptimizationTime1)
            .count();
  ...
  // saveMappingTime(vTimesInitialMapping, (result_dir_ /
  // "Initial_mapping_time.txt").string());
  // saveMappingTime(vTimesIncrementalMapping, (result_dir_ /
  // "Incremental_mapping_time.txt").string());
  // saveMappingTime(vTimesTailOptimization, (result_dir_ /
  // "Tail_optimization_time.txt").string());
...
cv::Mat GaussianMapper::renderFromPose(const Sophus::SE3f &Tcw, const int width,
  ...
  // viewer_main_undistort_mask_[pkf->camera_id_];
  // else
  // masked_image = std::get<0>(render_pkg) *
  // viewer_sub_undistort_mask_[pkf->camera_id_];
  masked_image = std::get<0>(render_pkg);
...
void GaussianMapper::saveMappingTime(const std::vector<double> &vTimesMapping,
  ...
  out << "Total time: " << std::fixed << std::setprecision(4) << totalTime
      << std::endl;
  out.close();
}
